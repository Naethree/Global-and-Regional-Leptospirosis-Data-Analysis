...
collection = db['srilanka_data']  # Collection name
...
def upload_csv_to_mongodb(csv_path, year):
    ...
    if os.path.exists(csv_path):
        ...
        for record in records:
            # Upsert operation: updates existing records or inserts new ones
            collection.update_one({'Year': year, 'Region': record.get('Region')}, {'$set': record}, upsert=True)
...
if today.month == 1 and today.day == 31:
    # Process data for the previous year (e.g., 2024)
